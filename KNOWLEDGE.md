...
│
└── assets/icons/            — SVG marker icons (colored-circle style, 36×36)
```
...
└── src/
    ...
    │   └── TripStatsPanel.js        # trip distance/duration statistics (legacy/unused)
    ├── core/
...
| OCP | `NearbyPlacesRenderer`: extend `PLACE_CATEGORIES` to add new place types. No class body changes needed |
...
| DIP | `MapController` accepts `TripRepository` as a constructor argument with sensible defaults — concrete implementations are injected |
...
- Loads trip data via `TripRepository`
- Creates and holds all renderer instances
- Manages trip highlight / deselect state
- Exposes public API for `App` to call: `selectTrip()`, `geocode()`, `renderPlannedRoute()`, etc.
...
| `<app-settings>` | `AppSettingsComponent` | Toggles: route arrows, terrain, dark mode |
| (no tag) | `NearbyPlacesPanel` | Category chips + nearby places list |
| (no tag) | `TripStatsPanel` | Distance/duration stats for selected trip (legacy/unused) |
...
| Active section | Trips visible | Planned route visible |
|---|---|---|
| `rides` | yes | no |
| `planner` | no | yes |
...
- `App.#onSectionChange()` calls `MapController.setTripLayersVisibility()` and
  `setPlannedRouteVisibility()` accordingly.
...
|---|---|---|---|---|
| `viewpoint` | `point_of_interest` + keyword | 🏔️ | Viewpoints | `viewpoint.svg` (green circle + mountain) |
| `tourist_places` | `tourist_attraction` + keyword "tourist places" | 📷 | Tourist Places | `emoji-tourist.svg` (purple circle + camera) |
| `cafe` | `cafe` | ☕ | Cafes | `cafe.svg` (amber circle + cup) |
| `restaurant` | `restaurant` | 🍽️ | Restaurants | `emoji-restaurant.svg` (orange circle + plate) |
| `hotel` | `lodging` | 🛏️ | Hotels | `emoji-hotel.svg` (blue circle + bed) |
| `museum` | `museum` | 🏛️ | Museums | `emoji-museum.svg` (pink circle + pillars) |
| `natural_feature` | `natural_feature` | 🌳 | Nature | `emoji-park.svg` (teal circle + tree) |
| `parking` | `parking` | 🅿️ | Parkings | `emoji-parking.svg` (gray circle + P) |
| `fuel` | `gas_station` | ⛽ | Fuel Stations | `fuel.svg` (orange circle + pump) |
| `campsite` | `campground` | ⛺ | Campsites | `emoji-campsite.svg` (green circle + tent) |

**Icon design rule:** All icons in `assets/icons/` are pure vector SVGs (no `<text>` or emoji characters). Google Maps loads icons as external images — emoji text fails to render in that context because there is no access to system emoji fonts. All icons use circles, polygons, rects, and paths only. Icons are 36×36 px, rendered at 32×32 on the map.
...
9. **"Open in Google Maps" per trip** — each trip's expanded details panel contains a link that opens the trip in Google Maps Directions (`https://www.google.com/maps/dir/?api=1&...`). The URL is built in `buildGoogleMapsUrl()` (top of `TripListComponent.js`) using the first waypoint as origin, last as destination, and all intermediate waypoints as pipe-separated `waypoints` param. Clicking the link does **not** trigger trip selection (click propagation is stopped).

10. **No trip editing** — trips are static JSON files. There is no in-app editor. To add/modify a trip, edit the JSON file directly and add it to the manifest.

11. **Map center is hardcoded to Zagreb** (`ZAGREB_CENTER = { lat: 45.8150, lng: 15.9819 }` in `MapController.js`). To fork the project for a different region, change this constant and the default zoom level.
